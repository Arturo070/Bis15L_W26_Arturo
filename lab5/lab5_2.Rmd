...
## Review  
In the previous lab, we used `select()` to extract columns of interest from a data frame. This helps us focus our attention on the variables arelevant to our question. However, it doesn't allow us to extract information in rows. The `filter()` function allows us to extract data in rows that meet specific criteria.  
...
```{r}
filter(penguins, body_mass_g> 5000, (bill_depth_mm>18 | bill_length_mm>50))
```
...
penguins %>% 
  select(species, body_mass_g) %>% 
  filter(species=="Gentoo") %>% 
  slice_min(body_mass_g, n=1) #dont forget the specify the variable
```
