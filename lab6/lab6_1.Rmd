...
```{r}
penguins %>% 
  filter(island %ln% c("Biscoe", "Dream"))
```
...
glimpse(mammals)
```
#a lot of the data is messed up (space in wean mass causing ' ' marks, forget to put space, special characters, capitals, )
...
```{r}
mammals %>% 
  mutate(across(everything(), tolower)) #be careful with numberic variables
```
