...
#This data is tidy because each variable is correctly formatted to its collumn as a long format. 
```{r}
head(sydneybeaches)
...
```{r}
syndeybeaches_wide<-sydneybeaches_long %>% 
...
sydneybeaches_long %>%
  ...
  group_by(year)%>%
  summarize(avg_ent=mean(enterococci_cfu_100ml, na.rm=T))
...
#The most polluted beach in 2013 was the Little Bay Beach. 

```{r}
sydneybeaches_long %>% 
  filter(year==2013) %>% 
  group_by(site) %>% 
  summarize(pollution=sum(enterococci_cfu_100ml)) %>% 
...
#This analysis tells use that the pollution is at its maximum at beach_id 28.0, descending in pollution level. 

```{r}
sydneybeaches %>% 
  select(-latitude,-longitude) %>% 
  group_by(beach_id) %>% 
  summarize(pollution=sum(enterococci_cfu_100ml,na.rm=T)) %>% 
  arrange(desc(pollution))
```
