...
\
#The study is testing whether the harvesting of wild elephant tusks had caused an decline in tusk size. Testing if wild harvesting of large tusks has led to smaller tusks in newer generations, thus seeing if tusk length is hertible. Using varibales of tusk size, homogeneity amoung groups, tusk length, circumfrence, geneder, and relative size among generations to test this. 
...
\
```{r}
elephants_f <- elephants_c %>% 
  mutate(years_of_sample_collection=as.factor
...
```{r}
elephants_f %>% 
  ggplot(mapping =  aes(x=years_of_sample_collection)) +
  geom_bar()
```
\
...
\
#Seems there is a positive relationship between tusk length and the estimated age. 
...
\
#Shoulder height does seem to be a positive predictor of tusk length, with the graph showing an upward trend as the heigh increases.
...
\
#This hypothesis is supported by the data as the average length has decreased between the sample years over time.

```{r}
ggplot(elephants_f, aes(x = years_of_sample_collection, y = tusk_length_in_cm)) +
  geom_boxplot() 
```
\
\
