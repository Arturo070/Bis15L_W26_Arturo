...
```{r}
heartrate %>% 
  ggplot(patient, mapping=aes(x=patient))
```
...
#Variables names are parts of the data. 
...
#Each day is its own collumn when its a variable
...
```{r}
qpcr_tidy %>% 
  separate(experiment, into=c("experiment","e_number"), sep=3) %>% separate(replicate, into=c("rep", "r_number"), sep=3) %>% 
