...
```{r}
summary(life_history)#Does the data make sense biologically? (Max life span is 0??, ,mean litter size is -999?)
```
...
```{r}
life_history %>% 
  summarize(across(everything(),~sum(is.na(.)))) #Tells us where NA is reresented 
```
