...
```{}
m genome01_blast.csv BIS15L_W2026
```
...
```{r}
#read_csv(data/blast_results/"Whatever we named it".csv")" %>% 
clean_names()
```
...
```{r}
#genome01_blastn %>% 
#  select(describe, e_value, accession) %>% 
...
```{r}
#readDNAStringSet("data/e.coli_cds.fasta")
```
