...
life_history %>% 
  count(order, sort=T)
#Helps you see what the X and Y is 
```
```{r}
life_history %>% 
  count(order, sort=T) %>% 
...
```{r}
life_history %>% 
  ggplot(aes(x=order, y=log10(mass)))+ #another way of scaling instead if scale log 10 in another layer 
...
life_history %>% 
  ...
  scale_x_log10()+
  geom_smooth(method = lm,se=F, na.rm=T)+
  labs(title="Ex", x="fkg", y="lpl")
#Clean but can be improved 
```
