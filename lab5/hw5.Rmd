...
**2. What is the structure of the `homerange` data? Does it have any NA's? Do a quick exploratory analysis of your choice below.** #Yes there does appear to be NA's in many of the quantitive data points. With it showcasing different species by their class, body size, and many other variables.
...
#The plot shows that there are more carnivores than there are herbivores.
...
```{r}
homerange %>% 
  select(species,common.name,order,mean.hra.m2,mean.mass.g)%>% 
  arrange(desc(mean.hra.m2))%>% 
  slice_head(n=2)
```
